...
本系统实现了"图文双重审核"机制，在原有文本审核的基础上增加了图像维度的智能合规性校验，提升平台内容安全标准。

## 核心特性

1. **文本审核**：使用 DeepSeek AI 对商品标题、描述等文本内容进行审核
2. **图像审核**：使用 CV 模型对用户上传的图片进行智能合规性校验
3. **综合评分**：文本占70%，图像占30%，综合计算最终审核分数
4. **自动决策**：
   - 综合分数 ≥ 80：自动通过审核
   - 综合分数 < 50：自动拒绝上架
   - 50 ≤ 综合分数 < 80：需要人工审核
...
2. 注册/登录阿里云账号
3. 开通"内容安全"服务
4. 创建 AccessKey ID 和 AccessKey Secret
5. 在 application.properties 中配置
...
**申请步骤：**
1. 访问 https://cloud.tencent.com/product/cms
2. 注册/登录腾讯云账号
3. 开通"内容安全"服务
4. 创建 API 密钥
5. 在 application.properties 中配置
